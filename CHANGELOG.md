...
- **Corrección: dos corridas de `actualizar.py` en paralelo duplicaban partidos.** Cada una lee los `fixture_id` ya cargados al arrancar, así que si la segunda empieza antes de que la primera escriba, las dos ven el mismo partido como nuevo. Pasó el 2 de agosto, cuando el cron de GitHub cayó a 52 segundos del disparo de Cloud Scheduler (16:59:29 y 17:00:21 UTC): 20 partidos duplicados sobre 37.106. El workflow pasa a tener grupo de concurrencia, con `cancel-in-progress: false` para que la segunda espere en vez de matar a la primera a mitad de escritura
