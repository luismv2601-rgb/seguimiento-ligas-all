...
## v1.9.0 - 2026-08-06
- **19 ligas europeas más: de 46 a 65 activas.** Todas las que arrancan en agosto, sumadas antes de su primera fecha para que midan desde el minuto cero: Países Bajos (88), Bélgica (144), Portugal (94), Bosnia (315) e Irlanda del Norte (408) el 7-ago; Turquía (203), Championship (40) y Malta (393) el 14; España (140), Kosovo (664) y Azerbaiyán (419) el 15; Francia (61) y Premier League (39) el 21; Italia (135), Grecia (197), Albania (310) e Israel (383) el 22; Alemania (78) el 28; Chipre (318) el 29. Entraron 10.922 partidos de baseline
- Inglaterra aporta dos ligas con el mismo país; la clave única es `liga_id`, así que no se pisan
- **Corrección: dos corridas de `actualizar.py` en paralelo se pisaban.** Pasó el 2 de agosto, cuando el cron de GitHub cayó a 52 segundos del disparo de Cloud Scheduler (16:59:29 y 17:00:21 UTC). El workflow pasa a tener grupo de concurrencia, con `cancel-in-progress: false` para que la segunda espere en vez de matar a la primera a mitad de escritura. Dejó dos daños distintos:
  - **20 partidos duplicados** en `Partidos`, sobre 37.106. Cada corrida lee los `fixture_id` ya cargados al arrancar, así que las dos vieron los mismos partidos como nuevos. Limpiados con `limpiar_duplicados.py`, que solo borra si la fila es idéntica campo por campo a la original
  - **38 filas de `Ranking_Empates` con datos de otro equipo**, y 2 filas repetidas. Es la pestaña que junta las dos condiciones: `actualizar.py` le escribe por número de fila y además la reordena por `pct_empates` al final, así que la segunda corrida escribió sobre filas que la primera ya había movido. 33 de las 38 llevaban valores de un equipo real distinto. Reconstruida entera desde `Partidos` con `reconstruir_ranking.py`. `Analisis` se reordena pero nunca recibe escrituras por fila, y `Racha_Actual` las recibe pero nunca se reordena: ninguna de las dos quedó afectada
- El límite del plan de API-Football quedó confirmado: **7.500 llamadas diarias**. Con 65 ligas se usan ~2.860, el 38%
- Cinco de las 19 entran ya en alerta (Kosovo 16/7, Irlanda del Norte 15/9, Albania 15/4, Malta 8/6, Bélgica 6/6). No es un error: la racha se cuenta como una secuencia continua entre temporadas, así que arrastran lo que traían del cierre de mayo
